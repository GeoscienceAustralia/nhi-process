...
def load_obs_tracks(trackfile: str, format: str) -> gpd.GeoDataFrame:
    ...
    if format == 'QC':
        ...
        colnames = ['NAME', 'num', 'datetime', 'lat','lon', 'pmin', 'poci', 'vmax']
        dtypes = [str, str, str, float, float, float, float, float]
...
def calculateCPA(stationFile: str, trackFile: str, trackFormat: str) -> gpd.GeoDataFrame:
    """
...
def extractObs(cpadf: pd.DataFrame, stations: pd.DataFrame) -> pd.DataFrame:
    outdf = pd.DataFrame(columns=['stnNum', 'stnName', 'dtObs', 'gust',
    ...
    # Load the data - need the state to determine offset from UTC
        ...
        for idx, tc in group.iterrows():
            ...
            try:
                indx = obsData.index.get_loc(tc.datetime, method='nearest', tolerance=pd.Timedelta('1D'))
            except KeyError:
                print(f"No obs within 1 day of {tc.datetime} at {stnNum} for {tc.NAME}")
                continue
            ...
            if obs.gust is not None:
                print(stnNum, obs.datetime, obs.gust, obs.direction, tc.datetime, tc.NAME, tc.cpa)
                outdf = outdf.append(
                    pd.DataFrame([[stnNum, stnName, obs.datetime, obs.gust, obs.direction, tc.datetime, tc.NAME, tc.cpa]],
                                columns=['stnNum', 'stnName', 'dtObs', 'gust', 'direction', 'dtTC', 'TCName', 'TCCPA']), 
                                ignore_index=True)
    return outdf
...
outdf.to_csv(r"X:\georisk\HaRIA_B_Wind\data\derived\tcobs\stncpa_obs.csv", index=False)

outdf.set_index('stnNum', inplace=True)
for stnNum, obs in outdf.groupby('stnNum'):
    if len(obs) > 10:
        print(stnNum, len(obs))
