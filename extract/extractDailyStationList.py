...
climatoldfnames = ["Filename", "Longitude", "Latitude", "Elevation", "StationNumber", "StationName"]

print(f"Loading data from {f}")
df = pd.read_csv(pjoin(dataPath,stnfile), sep=',', index_col=False, names=colnames, header=0,keep_default_na=False)
outdf = pd.DataFrame(columns = df.columns)
climatoldf = pd.DataFrame(columns=["Filename", "Longitude", "Latitude", "Elevation", "StationNumber", "StationName"])
for idx, row in df.iterrows():
    filename = f"DC02D_Data_{row.stnNum:06d}_999999999632559.txt"
    tmpdf = pd.read_csv(pjoin(dataPath, filename), sep=",", index_col=False, header=0, names=datacols, usecols=usecols, skipinitialspace=True)
    nrecords = tmpdf.windgust.notna().sum()
    if nrecords >= 365 * 5:
        outdf = outdf.append(row, ignore_index=True)
    if nrecords >= 365 * 10:
        climatoldf = climatoldf.append(pd.DataFrame([[pjoin(dataPath, filename), 
...
gdf = gpd.GeoDataFrame(outdf, geometry=gpd.points_from_xy(outdf.stnLon, outdf.stnLat, crs="EPSG:7844"))
gdf.to_file(pjoin(tempPath, "daily-stationlist.shp"))
gdf.to_file(pjoin(tempPath, "daily-stationlist.json"), driver='GeoJSON')
gdf.drop(columns=['geometry']).to_csv(pjoin(tempPath, "daily-stationlist.csv"), index=False)
